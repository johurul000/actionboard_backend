...
class ZoomOAuthStartView(APIView):
    ...
    def get(self, request):
        ...
        if not org_id:
            return Response({"error": "Missing org_id"}, status=400)
        
        user_id = request.user.id

        random_state = get_random_string(32)

        state_data = f"{user_id}:{org_id}:{random_state}"

        redirect_uri = "http://localhost:8000/api/integrations/zoom/oauth/callback/"
        client_id = settings.ZOOM_CLIENT_ID

        # Use state to pass org_id and a random string
...
class ZoomOAuthCallbackView(APIView):
    ...
    def get(self, request):
        ...
        try:
            user_id, org_id, random_state = state.split(":")
        except ValueError:
        ...
        try:
            ...
            return self.redirect_with_error("invalid_org_id")
        ...
        # Save or update the token in your OAuthToken model
        ...
        return redirect(f"http://localhost:3000/zoom-integration/success?org_id={org_id}")
